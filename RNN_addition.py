...
for j in range(20000):
    ...
    for position in range(binary_dim):
        ...
        # output layer
        output_layer = sigmoid(np.dot(hidden_layer, V))
    ...
    for position in range(binary_dim):
        ...
        hidden_delta = np.dot(future_hidden_layer_delta, W.T) + \
                    np.dot(output_delta, V.T) * sigmoid_prime(hidden_layer)
        #update d(loss)/dw(L) = a(L-1) * delta(L)
        V_update += np.dot(np.atleast_2d(hidden_layer).T, output_delta)
        W_update += np.dot(np.atleast_2d(prev_hidden_layer).T, hidden_delta)
    ...
    if j % 1000 == 0:
        ...
        out = 0
        # 2 --> 10 
        for idx, x in enumerate(reversed(c_hat)):
            out += x * pow(2, idx)
