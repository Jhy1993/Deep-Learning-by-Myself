...
for  i in  word_index.values:
    ...
    # print isinstance(word, "utf-8")
    embeddings_vector = embeddings_index.get(word)
    #print embeddings_vector
    if embeddings_vector is not None:
        embeddings_matrix[i] = embeddings_vector
print embeddings_matrix[400]
...
model.fit(x[:train_num], y[:train_num], batch_size = batch_size, verbose=True, nb_epoch=5)
 
score, acc = model.evaluate(x[train_num:], y[train_num:], batch_size = batch_size)
print('Test accuracy:', acc)
from keras.models import load_model
model.save('modle0812.h5')
