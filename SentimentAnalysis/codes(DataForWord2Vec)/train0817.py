...
for i in range(len(label)):
    ...
    if label[i] == 0:
        middle_index.append(label.index.get_loc(i))
    if label[i] == -1:
        neg_index.append(label.index.get_loc(i))
pos_num = len(pos_index)
middle_num = len(middle_index)
neg_num = len(neg_index)
def getmin(pos_num, middle_num, neg_num):
    min_num = pos_num
    ...
    if min_num > neg_num:
        min_num = neg_num
    return min_num
min_num = getmin(pos_num, middle_num, neg_num)  
if min_num < 500:
    print ('Data is too little, we need more data!!!!!!!!!')
pos = comment[pos_index[:min_num]]
middle = comment[middle_index[:min_num]]
neg = comment[neg_index[:min_num]]

pos_label = label[pos_index[:min_num]]
middle_label = label[middle_index[:min_num]]
neg_label = label[neg_index[:min_num]]

comment_new = pd.concat([pos, neg, middle], ignore_index=True)
label_new =  pd.concat([pos_label, neg_label, middle_label], ignore_index=True)
